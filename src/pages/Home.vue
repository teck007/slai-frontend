  ...
  <main class="flex-1">
    ...
    <section class="w-full py-12 md:py-20 lg:py-24">
      <div class="mx-auto flex max-w-3xl flex-col items-center gap-6 px-4 text-center md:px-6">
        <div class="space-y-3">
          <h1 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl md:text-5xl lg:text-6xl dark:text-gray-50">
            Acorta tus URLs con
            <span class="text-primary">IA</span>
          </h1>
          <p class="mx-auto max-w-2xl text-gray-500 md:text-lg dark:text-gray-400">
        ...
        <div class="w-full max-w-xl space-y-3">
          <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900">
            <form class="flex flex-col gap-3 sm:flex-row" @submit="handleShorten">
              <div class="relative flex-1">
                ...
                <input id="url-input" v-model="inputUrl" type="url" required placeholder="Pega tu enlace largo aquí"
                  ...
                  class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 pl-3 text-sm outline-none ring-primary/30 focus:border-primary focus:ring-2 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100" />
              </div>
            ...
            <div v-if="result" class="mt-4 space-y-2 text-sm">
              ...
              <div v-if="status !== 'error'" class="flex items-center gap-2 rounded-xl bg-gray-50 px-3 py-2 dark:bg-gray-800">
                <a :href="result" target="_blank" rel="noopener noreferrer"
                  class="flex-1 break-all font-semibold text-primary hover:underline dark:text-white">
                  {{ result }}
                </a>
                <button type="button" @click="copyResult" aria-label="Copiar URL acortada"
                  class="rounded-lg border border-gray-300 px-2 py-1 text-xs font-medium text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700">
                  Copiar
                </button>
              </div>
              <div v-if="status === 'error'" class="font-bold text-red-500 dark:text-red-400">
                {{ result }}
              </div>
            </div>
          </div>
    ...
    <section class="w-full">
      <div class="mx-auto grid max-w-5xl items-center gap-10 px-4 md:grid-cols-2 md:px-6">
        <div class="space-y-4">
          ...
          <p class="text-sm text-gray-600 md:text-base dark:text-gray-400">
        ...
        <div class="grid sm:grid-cols-2">
          <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900">
            ...
            <h3 class="font-semibold dark:text-white">Acorta URLs</h3>
          ...
          <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900">
            ...
            <h3 class="font-semibold dark:text-white">Copia y comparte</h3>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
